...
void main() {
    ...
    float fresnel = square_pow(1.0 + normal_dot_eye) * 0.6; // Reduced transparency from outside
    ...
    if(block_type > 2.5) {  // Water
            ...
            real_light = omni_light +
                ...
                block_color.rgb = water_texture * real_light * WATER_COLOR * 1.5; // Enhanced water color intensity
            #elif WATER_COLOR_SOURCE == 1
                block_color.rgb = 0.5 * water_texture * real_light * tint_color.rgb; // Increased opacity
            #endif
