...
void main() {
    #if (defined SHADOW_CASTING && !defined NETHER) || defined DISTANT_HORIZONS
        #if AA_TYPE > 0 
            float dither = shifted_dither_reflexa(gl_FragCoord.xy);
        #else
    ...
    #if defined GBUFFER_TERRAIN || defined GBUFFER_HAND
        if(emmisive_type > 0.1) {
            final_candle_color *= block_luma * 1.3;
            // Enhanced emissive glow with better color preservation
            float glow_strength = mix(1.2, 2.8, min(emmisive_type, 1.0));
            block_color.rgb *= glow_strength;
            // Enhanced color range for better visibility
            block_color.rgb = clamp(block_color.rgb, vec3(0.15), vec3(3.5));
        }
    #endif
