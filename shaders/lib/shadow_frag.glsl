...
float get_shadow(vec3 the_shadow_pos, float dither) {
    float shadow_sample = 1.0;
        ...
        // Enhanced shadow sampling for better detail
        ...
        vec2 offset_3 = offset * 0.5;
        vec2 offset_4 = offset_2 * 0.5;

        float z_bias = dither * 0.00001; // Reduced Z-fighting for sharper shadows

        // Enhanced 8-point sampling for detailed shadows
        ...
        shadow_sample += shadow2D(shadowtex1, vec3(the_shadow_pos.xy - offset_2, the_shadow_pos.z - z_bias)).r;
        shadow_sample += shadow2D(shadowtex1, vec3(the_shadow_pos.xy + offset_3, the_shadow_pos.z - z_bias)).r;
        shadow_sample += shadow2D(shadowtex1, vec3(the_shadow_pos.xy - offset_3, the_shadow_pos.z - z_bias)).r;
        shadow_sample += shadow2D(shadowtex1, vec3(the_shadow_pos.xy + offset_4, the_shadow_pos.z - z_bias)).r;
        shadow_sample += shadow2D(shadowtex1, vec3(the_shadow_pos.xy - offset_4, the_shadow_pos.z - z_bias)).r;

        shadow_sample *= 0.125; // Normalize for 8 samples
    #endif
