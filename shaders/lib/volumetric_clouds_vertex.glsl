...
cloud_color = mix(
    clamp(mix(vec3(luma(cloud_color_aux)), cloud_color_aux, 0.6 * CLOUD_DENSITY * CLOUD_CONTRAST) * vec3(1.8 * CLOUD_BRIGHTNESS), 0.0, 1.6),
    day_blend(
        HORIZON_SUNSET_COLOR,
        HORIZON_DAY_COLOR,
        HORIZON_NIGHT_COLOR
    ),
    0.25 * CLOUD_COVERAGE * CLOUD_DEPTH_MULTIPLIER
);

cloud_color = mix(cloud_color, HORIZON_SKY_RAIN_COLOR * luma(cloud_color_aux) * 5.0, rainStrength);

dark_cloud_color = mix(dark_cloud_color, cloud_color * CLOUD_SHADOW_INTENSITY, 0.28 * CLOUD_DENSITY * CLOUD_CONTRAST);
